...
for alpha in "${alphas[@]}"; do
    for seed in "${seeds[@]}"; do
        for method in "${methods[@]}"; do
            python train.py --method ${method} --non-iid --seed ${seed} --split-coef ${alpha}
        done
    done
done
            ...
            for m in "${methods[@]}"; do
                for g in "${generators[@]}"; do
                    for n in "${num_data[@]}"; do
                        python train.py --method ${m} --non-iid --syn --generator ${g} --num_per_class ${n} --early --patience ${p} --seed ${seed} --split-coef ${alpha}
                    done
                done
            done
        done
